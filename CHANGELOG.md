...
- 照明 / 空调独立开关：设置页「基础运行 → 监控范围」可为每块表单独启用或关闭，只查其中一块。开关状态存在 `config.json` 的 `meters[].enabled`，默认 `true`，旧配置自动视为全启用。
- 已关闭的电表不再发起查询、不参与阈值判定、不发提醒，总览页与轮值摘要也不再显示。
- 至少保留一块表启用：全部关闭时会拒绝操作并提示，避免无意义的空轮询。
- 全部关闭时明确记录「未查询」日志，不再误报为查询失败。

### 安全修复（重要）

- 移除示例配置与 README 中的真实 `openId`，改为打码占位值。此前该值自 v0.1.0 起明文存在于公开仓库。
- 修复打包逻辑：`desktop/package.json` 原本把根目录 `config.json` 作为 extraResources 打进安装包，导致开发者本机发件邮箱、SMTP 授权码与收件人随 v0.1.0 / v0.1.1 / v0.1.2 的 exe 一并分发。现改为只打包 `config.example.json`，Electron 主进程首次启动仅用示例配置兜底。
- 移除默认弱口令 `123456`（原在示例配置、README 与 `config.py` 兜底逻辑中）。未设置 `security.adminPassword` 时，敏感设置与邮件模板无法解锁。
- 重写 Git 历史，清除全部提交中的明文凭证。

### 注意

- v0.2.0 之前的安装包内含泄露的 SMTP 授权码，已删除旧 release 附件。**请务必到邮箱后台吊销旧授权码并重新生成**；仅在应用内换密码无法让已泄露的凭证失效。
